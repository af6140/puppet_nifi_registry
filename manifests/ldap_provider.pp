#
define nifi_registry::ldap_provider (
  ...
  concat::fragment { "id_frag_${identifier}":
    order   => '02',
    target  => "${conf_dir}/identity-providers.xml",
    content => template('nifi_registry/idmapping/frag_ldap_provider.erb'),
  }

  nifi_registry::config_properties {'nifi_registry_properties_ldap_provider':
    conf_dir => $conf_dir,
    properties => {
